                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <div class="fw-bold">{{ $record->patient->name ?? 'No Patient' }}</div>
                                    <small class="text-muted">
                                        {{ $record->patient ? \Carbon\Carbon::parse($record->patient->birth_date)->age . ' years old' : 'N/A' }}
                                    </small>
                                </div>
                            </div>
                        </td>
                            ...
                            <div class="d-flex align-items-center">
                                <div class="avatar-sm me-2">
                                    <div class="avatar-title rounded-circle bg-primary text-white">
                                        {{ substr($record->doctor->name ?? 'N/A', 0, 1) }}
                                    </div>
                                </div>
                                <div>
                                    <div class="fw-bold">{{ $record->doctor->name ?? 'No Doctor' }}</div>
                                    <small class="text-muted">{{ $record->doctor->specialization ?? 'N/A' }}</small>
                                </div>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-4">
                            <div class="text-muted">
                                ...
                                <p>No medical records found. <a href="{{ route('admin.medical-records.create') }}">Add the first record</a></p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
...
function printRecord(recordId) {
    window.open(`/admin/medical-records/${recordId}/print`, '_blank');
}
</script>
